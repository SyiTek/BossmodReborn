...
class Un5ThordanStates : StateMachineBuilder
{
    ...
    private void SinglePhase(uint id)
    {
        ...
        Targetable(id + 0xC0000, false, 2f, "Boss disappears");

        // intermission
        HeavensflameChainsConviction(id + 0x100000, 16.2f); // note: quite large variance
        SacredCrossSpiralThrust(id + 0x110000, 3f);
        AdelphelJanlenoux(id + 0x120000, 7f);
        ...
        Cast(id + 0x2D0000, (uint)AID.Enrage, 2.1f, 10f, "Enrage");
    }
    ...
    private void DragonsEyeGaze(uint id, float delay)
    {
        ...
        Cast(id + 0x10, (uint)AID.DragonsGaze, 7.2f, 3f, "Gaze")
    ...
    private void HeavensflameChainsConviction(uint id, float delay)
    {
        ...
        ComponentCondition<Conviction>(id + 0x10, 2f, comp => comp.Towers.Count > 0)
            .ActivateOnEnter<BurningChains>()
            .ActivateOnEnter<Conviction>();
        // +0.1s: holy chain cast?..
        ComponentCondition<Heavensflame>(id + 0x20, 4f, comp => comp.Casters.Count == 0, "Puddles resolve")
    ...
    private void SacredCrossSpiralThrust(uint id, float delay)
    {
        ...
        ComponentCondition<SpiralThrust>(id + 0x20, 6f, comp => comp.NumCasts > 0, "Lines")
    ...
    private void AdelphelJanlenoux(uint id, float delay)
    {
        ...
        ComponentCondition<SwordShieldOfTheHeavens>(id + 0x1000, 100f, comp => !comp.Active, "Adds enrage", 10000f)
    ...
    private void SpiralPierceDimensionalCollapseHiemalStormComets(uint id, float delay)
    {
        ...
        ComponentCondition<MeteorCircle>(id + 0x1010, 50f, comp => comp.ActiveActors.Count == 0, "Large comet enrage", 10000f) // TODO: proper time
    ...
    private void KnightsOfTheRound1(uint id, float delay)
    {
        ...
        Cast(id + 0x10, (uint)AID.KnightsOfTheRound, 7.2f, 3f);
        AscalonsMight(id + 0x20, 5.2f);
        ComponentCondition<HolyShieldBash>(id + 0x30, 6f, comp => comp.NumCasts > 0, "Stun healer")
            .ActivateOnEnter<HolyShieldBash>();
        ComponentCondition<HolyShieldBash>(id + 0x40, 3.1f, comp => comp.Source != null);
        CastStart(id + 0x41, (uint)AID.HeavenlyHeel, 5f);
        ComponentCondition<HolyShieldBash>(id + 0x42, 1f, comp => comp.NumCasts > 1, "Wild charge")
            .DeactivateOnExit<HolyShieldBash>();
        CastEnd(id + 0x43, 3f, "Tankbuster")
            .SetHint(StateMachine.StateHint.Tankbuster);
        Cast(id + 0x50, (uint)AID.DragonsGaze, 2.1f, 3f, "Gaze")
            .DeactivateOnExit<DragonsGaze>();
    }
    ...
    private void KnightsOfTheRound2(uint id, float delay)
    {
        ...
        ComponentCondition<Conviction>(id + 0x20, 7.2f, comp => comp.Towers.Count > 0)
            .ActivateOnEnter<Conviction>();
        Cast(id + 0x30, (uint)AID.DragonsGaze, 1f, 3f, "Gaze")
            .ActivateOnEnter<HeavyImpact>() // starts ~1s into cast
            .DeactivateOnExit<DragonsGaze>();
        ComponentCondition<HeavyImpact>(id + 0x40, 1.1f, comp => comp.NumCasts > 0, "Ring 1");
        ComponentCondition<HeavyImpact>(id + 0x50, 2f, comp => comp.NumCasts > 1, "Towers + Ring 2")
            .DeactivateOnExit<Conviction>(); // happen at the same time
        ComponentCondition<DimensionalCollapse>(id + 0x60, 1f, comp => comp.Casters.Count > 0)
            .ActivateOnEnter<DimensionalCollapse>();
        ComponentCondition<HeavyImpact>(id + 0x70, 1f, comp => comp.NumCasts > 2, "Ring 3");
        ComponentCondition<HeavyImpact>(id + 0x80, 2f, comp => comp.NumCasts > 3, "Ring 4")
            .DeactivateOnExit<HeavyImpact>();
        ComponentCondition<DimensionalCollapse>(id + 0x90, 3f, comp => comp.NumCasts > 0, "Puddles")
            .DeactivateOnExit<DimensionalCollapse>();

        CastStart(id + 0xA0, (uint)AID.DragonsRage, 0.2f)
            .ActivateOnEnter<FaithUnmoving>();
        ComponentCondition<FaithUnmoving>(id + 0xA1, 3f, comp => comp.NumCasts > 0, "Knockback")
            .ActivateOnEnter<DragonsRage>()
            .DeactivateOnExit<FaithUnmoving>();
        CastEnd(id + 0xA2, 2f, "Stack")
            .DeactivateOnExit<DragonsRage>();
    }
    ...
    private void KnightsOfTheRound4(uint id, float delay)
    {
        ...
        ComponentCondition<Heavensflame>(id + 0x31, 1f, comp => comp.Casters.Count > 0, "Puddles bait")
            .ActivateOnEnter<Heavensflame>();
        CastEnd(id + 0x32, 2f, "Gaze")
    ...
    private void KnightsOfTheRound5(uint id, float delay)
    {
        ...
        Cast(id + 0x10, (uint)AID.KnightsOfTheRound, 7.2f, 3f);
        AscalonsMight(id + 0x20, 5.2f);
        ComponentCondition<HoliestOfHoly>(id + 0x30, 5f, comp => comp.NumCasts > 0, "Raidwide")
            .ActivateOnEnter<HoliestOfHoly>()
            .DeactivateOnExit<HoliestOfHoly>()
            .SetHint(StateMachine.StateHint.Raidwide);
        ...
        ComponentCondition<HeavenswardLeap>(id + 0x50, 6.1f, comp => comp.NumCasts > 0, "Raidwide 1")
            ...
            .SetHint(StateMachine.StateHint.Raidwide);
        ComponentCondition<HeavenswardLeap>(id + 0x51, 3f, comp => comp.NumCasts > 1, "Raidwide 2")
            .SetHint(StateMachine.StateHint.Raidwide);
        ComponentCondition<HeavenswardLeap>(id + 0x52, 3f, comp => comp.NumCasts > 2, "Raidwide 3")
